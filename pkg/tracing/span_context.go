...
func ChildSpanFromEmbeddableSpanContext(parent ot.Span, name, value string) (ot.Span, error) {
	data, err := base64.StdEncoding.DecodeString(value)
	if err != nil {
		return nil, err
	}
	...
	return ot.StartSpan(name, ot.ChildOf(parent.Context()), ot.FollowsFrom(spanContext)), nil
}

// SpanContextFromContext returns a string which can be put in an annotation,
// or a blank string if something went wrong or there is no current span.
